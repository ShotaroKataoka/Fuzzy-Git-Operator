...
function _fgo_gitbranch_widget() {
  ...
  if [ -n "$_is_git_dir" ]; then
    ...
    ## FZF
    selected_branch=$(echo $git_branchs | sed -e '/\*/d' | cut -d " " -f 3 | fzf --cycle +m --info='inline' --layout=reverse --border --prompt='Git Switch >> ' --height=70% --bind="$GENERAL_KEYBIND_BH" --preview "git show --color=always {}" --color="$_FGO_COLOR_SCHEME")
...
function _fgo_gitpull_widget() {
  ...
  if [ -n "$_is_git_dir" ]; then
    local git_branchs=$(git branch)
    ## FZF
    selected_branch=$(echo $git_branchs | sed 's/\*/ /' | cut -d " " -f 3 | fzf --cycle +m --info='inline' --layout=reverse --border --prompt='Git Pull >> ' --height=70% --bind="$GENERAL_KEYBIND_BH" --preview "git show --color=always {}" --color="$_FGO_COLOR_SCHEME")
    if [ -n "$selected_branch" ]; then
