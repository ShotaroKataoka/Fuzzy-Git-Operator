...
function _fzf_gitadd_widget() {
  ...
  if [ -n "$_is_git_dir" ]; then
    ...
    do
      ...
      local git_selected=$(echo $git_status | fzf --ansi --cycle --bind="alt-h:abort,alt-j:down,alt-k:up,alt-l:accept,left:abort,right:accept,alt-c:abort,ctrl-h:abort,ctrl-j:preview-down,ctrl-k:preview-up,ctrl-l:accept,alt-i:toggle-preview,ctrl-i:toggle-preview" --preview="echo {} | rev | cut -f 1 -d ' ' | rev | xargs -rI{a} sh -c 'batcat {a} --color=always'")
      if [ -n "$git_selected" ]; then
        if [ -n "$(echo "$git_selected" | grep -e "^ M" -e "^ D" -e "^MM" -e "^??")" ]; then
          git add $(echo "$git_selected" | rev | cut -f 1 -d ' ' | rev)
        fi
        if [ -n "$(echo "$git_selected" | grep -e "^M " -e "^D " -e "^A ")" ]; then
          git reset HEAD $(echo "$git_selected" | rev | cut -f 1 -d ' ' | rev)
        fi
      else
        break
      fi
